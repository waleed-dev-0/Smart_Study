  ...
  try {
    ...
    if (!document_id || !mongoose.Types.ObjectId.isValid(document_id)) {
      return res.status(400).json({
        success: false,
        message: "Invalid or missing data: document_id must be a valid ObjectId"
      });
    }

    const chunks = await DocumentChunk.find({ document_id }).sort('chunk_index').limit(15);
    console.info(`[Quiz Generator] Found ${chunks.length} chunks for document ${document_id}`);
    ...
    if (!chunks || chunks.length === 0) {
      ...
      if (!document || !document.file_path || !fs.existsSync(document.file_path)) {
        return res.status(404).json({
          success: false,
          message: "No text found for this document. Please re-upload the file."
        });
      }
      ...
      try {
        ...
        if (!data || !data.text || data.text.trim().length === 0) {
          return res.status(404).json({ success: false, message: 'No extractable text found in the PDF.' });
        }

        // Smart slice: Take a large chunk but avoid random cutting mid-sentence
        ...
        return res.status(500).json({ success: false, message: 'Failed to extract text from the document.' });
      }
    } else {
    ...
    const questions = await questionService.extractQuestionsFromText(contentToProcess, numQuestions, difficulty, language);
    ...
    const validQuestions = questions.filter(
      (q) =>
        typeof q.question_text === 'string' && q.question_text.trim().length > 0 &&
    ...
    console.info(`[Quiz Generator] Successfully generated and saved ${saved.length} questions.`);
